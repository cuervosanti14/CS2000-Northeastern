# """
# Program #2 using Scrooge's list of employees to
...
def total_salary():
    ...
    for i in range(len(lst_ages)):
        if lst_ages[i] >= 25:
            total += lst_salaries[i]
    # Output (total salary)
...
def is_exist(name):
    ...
    for i in range(len(lst_names)):
        ...
        if name == lst_names[i]:
            # Return index if name in list + print name in list
            print(name, "is in the list")
            return i
    # Return -1 if name not in list + print name not in list
...
def increase_salary():
    ...
    if position != -1:
        lst_salaries[position] *= 1.2
        print(employee, "new salary =", lst_salaries[position])
    else:
...
def highest_salary():
    ...
    for i in range(len(lst_salaries)):
        if lst_salaries[i] > max:
            max = lst_salaries[i]
            name_max = lst_names[i]
    # Output (name and salary of employee w/ highest salary)
